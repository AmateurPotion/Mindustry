...
{
  internal sealed class XXHash32JavaUnsafe : XXHash32
  {
    [Modifiers]
    ...
    public override int hash([In] byte[] obj0, [In] int obj1, [In] int obj2, [In] int obj3)
    {
      ...
      if (obj2 >= 16)
      {
        ...
        do
        {
          ...
          obj1 += 4;
          num7 = Integer.rotateLeft(num7 + UnsafeUtils.readIntLE(obj0, obj1) * -2048144777, 13) * -1640531535;
          obj1 += 4;
        }
        while (obj1 <= num3);
      ...
      for (; obj1 <= num1 - 4; obj1 += 4)
        num8 = Integer.rotateLeft(num8 + UnsafeUtils.readIntLE(obj0, obj1) * -1028477379, 17) * 668265263;
      for (; obj1 < num1; ++obj1)
        num8 = Integer.rotateLeft(num8 + ((int) (sbyte) UnsafeUtils.readByte(obj0, obj1) & (int) byte.MaxValue) * 374761393, 11) * -1640531535;
    ...
    [LineNumberTable(18)]
    [MethodImpl(MethodImplOptions.NoInlining)]
    static XXHash32JavaUnsafe()
    {
      if (ByteCodeHelper.isAlreadyInited("net.jpountz.xxhash.XXHash32JavaUnsafe"))
        return;
      XXHash32JavaUnsafe.INSTANCE = (XXHash32) new XXHash32JavaUnsafe();
    }
  }
}
