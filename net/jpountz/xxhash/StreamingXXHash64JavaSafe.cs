...
// Type: net.jpountz.xxhash.StreamingXXHash64JavaSafe
...
{
  internal sealed class StreamingXXHash64JavaSafe : AbstractStreamingXXHash64Java
  {
    [LineNumberTable(new byte[] {159, 169, 103})]
    [MethodImpl(MethodImplOptions.NoInlining)]
    internal StreamingXXHash64JavaSafe([In] long obj0)
      : base(obj0)
    {
    }
    ...
    public override long getValue()
    {
      ...
      int i;
      for (i = 0; i <= this.memSize - 8; i += 8)
      {
        long num3 = Long.rotateLeft(SafeUtils.readLongLE(this.memory, i) * -4417276706812531889L, 31) * -7046029288634856825L;
        num2 = Long.rotateLeft(num2 ^ num3, 27) * -7046029288634856825L + -8796714831421723037L;
      }
      if (i <= this.memSize - 4)
      {
        num2 = Long.rotateLeft(num2 ^ ((long) SafeUtils.readIntLE(this.memory, i) & (long) uint.MaxValue) * -7046029288634856825L, 23) * -4417276706812531889L + 1609587929392839161L;
        i += 4;
      }
      for (; i < this.memSize; ++i)
        num2 = Long.rotateLeft(num2 ^ (long) this.memory[i] * 2870177450012600261L, 11) * -7046029288634856825L;
    ...
    public override void update([In] byte[] obj0, [In] int obj1, [In] int obj2)
    {
      ...
      else
      {
        ...
        {
          num3 = Long.rotateLeft(num3 + SafeUtils.readLongLE(obj0, obj1) * -4417276706812531889L, 31) * -7046029288634856825L;
          obj1 += 8;
    ...
    internal new class Factory : Object, StreamingXXHash64.Factory
    {
      ...
      [LineNumberTable(21)]
      [MethodImpl(MethodImplOptions.NoInlining)]
      public virtual StreamingXXHash64 newStreamingHash([In] long obj0) => (StreamingXXHash64) new StreamingXXHash64JavaSafe(obj0);

      [LineNumberTable(17)]
      [MethodImpl(MethodImplOptions.NoInlining)]
      static Factory()
      {
        if (ByteCodeHelper.isAlreadyInited("net.jpountz.xxhash.StreamingXXHash64JavaSafe$Factory"))
          return;
        StreamingXXHash64JavaSafe.Factory.INSTANCE = (StreamingXXHash64.Factory) new StreamingXXHash64JavaSafe.Factory();
      }
    }
  }
}
